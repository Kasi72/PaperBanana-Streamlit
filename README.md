...
## PaperBanana Streamlit Studio

This repository includes a polished Streamlit workspace for generating, comparing, downloading, and refining scientific figures with the real PaperBanana pipeline.

### Run locally

On Windows, double-click `start_app.cmd`. It selects Python 3.12 or 3.11, creates a private `.venv`, installs the dependencies on first launch, and starts Streamlit.

To start it manually on any platform, use a Python 3.11 or 3.12 environment:
...
Do not launch `streamlit_app.py` with bare `python`; Streamlit apps must be started with `streamlit run` (or the Windows launcher above).

Each visitor chooses **OpenRouter** or **Gemini** and enters their own API key in the password-masked session field. The Streamlit app does not write visitor keys to files. Because the upstream provider clients are process-global, separate visitors' generation jobs are safely serialized while candidates inside one job remain parallel.

### Deploy on Streamlit Community Cloud

1. Fork or push this repository to your GitHub account.
2. On the **Deploy an app** page, enter:
   - **Repository:** `<your-github-user>/<repository-name>`
   - **Branch:** `main` (or the branch containing this app)
   - **Main file path:** `streamlit_app.py`
   - **App URL:** optional
3. Click **Deploy**. No Streamlit Secrets are required because visitors provide their own keys per session.

The app targets Python 3.12 through `runtime.txt`. Community Cloud storage is ephemeral, so generated candidates are kept in session memory and exported through PNG/ZIP downloads.

> [!IMPORTANT]
> The upstream project is Apache-2.0 licensed, but its disclaimer says related workflow patents restrict third-party commercial applications. Review that disclaimer before commercial deployment.
